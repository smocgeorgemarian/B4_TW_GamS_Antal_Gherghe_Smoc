CREATE OR REPLACE PACKAGE rewards AS
    ...
    FUNCTION get_level(hash_code VARCHAR2, xp FLOAT) RETURN VARCHAR2;
    PROCEDURE update_level(hash_code IN VARCHAR2, user_name IN VARCHAR2, lvl IN VARCHAR2);
END rewards;
...
CREATE OR REPLACE PACKAGE BODY rewards AS
    ...
    BEGIN
        ...
        LOOP 
            IF DBMS_SQL.FETCH_ROWS(v_cursor_id)>0 THEN 
                ...
                IF verify_condition(condition, hash_code, user_name) = 1 AND test_existence.test_reward_user(reward_name, hash_code,user_name) = 0 THEN
    ...
    FUNCTION get_level(hash_code VARCHAR2, xp FLOAT)
    RETURN VARCHAR2 AS
        v_cursor_id INTEGER;
        v_ok INTEGER;
        v_command VARCHAR2(500);
        ...
        lvl_max VARCHAR2(200) := 'NULL';
        value_max FLOAT := 0;
        
        lvl_name VARCHAR2(200);
        level_value FLOAT;
    BEGIN
    
        v_table_name := 'LEVEL_' || hash_code;
        v_command := 'SELECT lname, lvalue FROM ' || v_table_name;
        v_cursor_id := DBMS_SQL.OPEN_CURSOR;
        DBMS_SQL.PARSE(v_cursor_id, v_command, DBMS_SQL.NATIVE);
        DBMS_SQL.DEFINE_COLUMN(v_cursor_id, 1, lvl_name, 200); 
        DBMS_SQL.DEFINE_COLUMN(v_cursor_id, 2, level_value); 
        v_ok := DBMS_SQL.EXECUTE(v_cursor_id);
        LOOP 
            IF DBMS_SQL.FETCH_ROWS(v_cursor_id)>0 THEN 
                DBMS_SQL.COLUMN_VALUE(v_cursor_id, 1, lvl_name); 
                DBMS_SQL.COLUMN_VALUE(v_cursor_id, 2, level_value); 

                IF xp >= level_value AND level_value >= value_max THEN
                    value_max := level_value;
                    lvl_max := lvl_name;
                END IF;
    ...
    PROCEDURE update_level(hash_code IN VARCHAR2, user_name IN VARCHAR2, lvl IN VARCHAR2) AS
        v_cursor_id INTEGER;
        v_ok INTEGER;
        v_command VARCHAR2(500);
        v_table_name VARCHAR2(200);
    BEGIN
        ...
        IF test_existence.test_level_user(hash_code, user_name) = 1 THEN
            ...
            v_command := 'UPDATE ' || v_table_name || ' SET xplevel = :xlevel WHERE user_name = :xname';
            v_cursor_id := DBMS_SQL.OPEN_CURSOR;
            DBMS_SQL.PARSE(v_cursor_id, v_command, DBMS_SQL.NATIVE);
            DBMS_SQL.BIND_VARIABLE(v_cursor_id, ':xlevel', lvl);
